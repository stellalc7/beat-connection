...
<b>TLDR: I tried Spotify's API, and it didn't work, but here's the code I had written.</b>
```js
...
https://ci.italy.tx.us/" target="_blank">there's a small town in Texas called Italy</a>. So there isn't an error per se, but I'm fetching local conditions in Italy, TX with a stream from the country Italy (proabably - most likely).<br>
...
In conclusion, if a city name does not coexist with that of a country's, the country will be mapped. And I know you're also wondering what if there are multiple cities with the same name (Versailles), the one everyone's probably thinking of will be mapped.
